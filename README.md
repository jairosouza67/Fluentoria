...
```bash
cd c:\Users\jairo\dyad-apps\Fluentoria
```
...
1. Create Firestore composite indexes for:
   - `studentId` + `timestamp` (Student Activities)
   - `courseId` + `uploadedAt` (Course Media)
...
// Automatically syncs user data to Firestore
// Creates user document with initial role
```
...
// - Module shortcuts (Courses, Daily, Mindful, Music)
...
// - Course description with expandable content
// - Course chat for instructor interaction
// - Media upload for assignments
// - Progress tracking
```
...
// Features:
// - Unique completion tracking per calendar day
...
// Gamification system
// Displays:
// - Unlocked achievements with XP rewards
// - Level progress
...
```typescript
// User Management
getUserRole(uid: string): Promise<'admin' | 'student'>
setUserRole(uid: string, role: 'admin' | 'student'): Promise<void>
forceUpdateUserRole(uid: string, email: string): Promise<void>
...
// Student Activity
logActivity(userId: string, activity: StudentActivity): Promise<void>
getStudentActivities(studentId: string, limit?: number): Promise<StudentActivity[]>

// Course Media
getCourseMedia(courseId: string): Promise<MediaSubmission[]>
addMediaSubmission(submission: MediaSubmission): Promise<void>
```

### gamification.ts - Achievement & XP System
```typescript
// Achievement System
checkAndUnlockAchievements(studentId: string): Promise<Achievement[]>
getStudentProgress(studentId: string): Promise<StudentProgress>
updateStudentXP(studentId: string, xpAmount: number): Promise<void>
...
```typescript
// Media Upload
uploadMediaFile(file: File, userId: string, courseId: string): Promise<string>
...
```typescript
// Attendance
logAttendance(studentId: string, studentName: string): Promise<void>
...
getCourseMessages(courseId: string): Promise<Message[]>
```
